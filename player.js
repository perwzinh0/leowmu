...
function initializePlayer(client) {
    ...
    client.riffy.on("nodeError", (node, error) => {
        console.error(`Node "${node.name}" encontrou um erro: ${error.message}.`);
    });
    ...
    client.riffy.on("trackStart", async (player, track) => {
        ...
        await channel.send({ embeds: [loopEmbed] });
            ...
            .setTitle("**O bot tocará a próxima música!**")
            .setTimestamp();
...
for (let i = 0; i < pages.length; i++) {
    ...
    const queueEmbed = new EmbedBuilder()
        .setColor("#0099ff")
        .setTitle(`Fila Atual (Página ${i + 1}/${pages.length})`)
    ...
    client.riffy.on("queueEnd", async (player) => {
        ...
        if (autoplay) {
            ...
            const queueEmbed = new EmbedBuilder()
                .setColor("#0099ff")
                .setDescription('**A fila de músicas terminou! Desconectando o bot!**');
